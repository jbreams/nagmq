...
void* getsock(char* forwhat, int type, json_t* def) {
    ...
    if (get_values(def,
                   ...
                   &linger,
                   NULL) != 0) {
        logit(NSLOG_CONFIG_ERROR, TRUE, "Invalid parameters for creating %s NagMQ socket", forwhat);
        return NULL;
    }
    ...
    if (sndhwm > 0 && zmq_setsockopt(sock, ZMQ_SNDHWM, &sndhwm, sizeof(sndhwm)) != 0) {
        logit(NSLOG_RUNTIME_ERROR,
              TRUE,
              "NagMQ error setting send HWM for %s: %s",
              forwhat,
              zmq_strerror(errno));
        zmq_close(sock);
        return NULL;
    ...
    if (rcvhwm > 0 && zmq_setsockopt(sock, ZMQ_RCVHWM, &rcvhwm, sizeof(sndhwm)) != 0) {
        logit(NSLOG_RUNTIME_ERROR,
              TRUE,
              "NagMQ error setting receive HWM for %s: %s",
              forwhat,
              zmq_strerror(errno));
        zmq_close(sock);
        return NULL;
    ...
    if (backlog > 0 && zmq_setsockopt(sock, ZMQ_BACKLOG, &backlog, sizeof(backlog)) != 0) {
        logit(NSLOG_RUNTIME_ERROR,
              TRUE,
              "NagMQ error setting connection backlog for %s: %s",
              forwhat,
              zmq_strerror(errno));
        zmq_close(sock);
        return NULL;
    ...
    if (maxmsgsize > 0 &&
        ...
        logit(NSLOG_RUNTIME_ERROR,
              TRUE,
              "NagMQ error setting maximum message size for %s: %s",
              forwhat,
              zmq_strerror(errno));
        zmq_close(sock);
        return NULL;
    ...
    if (accept_filters) {
        ...
        for (i = 0; i < len; i++) {
            ...
            if (!filter) {
                logit(NSLOG_CONFIG_ERROR,
                      TRUE,
                      "NagMQ TCP accept filter %lu for %s is not a string",
                      i,
                      forwhat);
                zmq_close(sock);
                return NULL;
    ...
    if (curve_privatekey) {
        int yes = 1, rc;
        rc = zmq_setsockopt(sock, ZMQ_CURVE_SECRETKEY, curve_privatekey, strlen(curve_privatekey));
        if (rc == -1) {
            logit(NSLOG_RUNTIME_ERROR,
                  TRUE,
        ...
        rc = zmq_setsockopt(sock, ZMQ_CURVE_PUBLICKEY, curve_privatekey, strlen(curve_publickey));
        if (rc == -1) {
            logit(NSLOG_RUNTIME_ERROR,
                  TRUE,
                  "Error setting NagMQ public key for %s %s %s",
                  forwhat,
                  curve_privatekey,
                  zmq_strerror(errno));
            zmq_close(sock);
    ...
    if (connect && zmq_connect(sock, connect) != 0) {
        logit(NSLOG_RUNTIME_ERROR,
              TRUE,
              "NagMQ error connecting %s to %s: %s",
              forwhat,
              connect,
              zmq_strerror(errno));
        zmq_close(sock);
