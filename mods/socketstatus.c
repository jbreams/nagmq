...
int sock_monitor_cb(int sd, int events, void* sock) {
    while (1) {
        ...
        if (rc == -1) {
            if (errno == EAGAIN || errno == ETERM)
                ...
                logit(NSLOG_RUNTIME_ERROR,
                      TRUE,
                      "Error receiving socket monitor message %s",
                      zmq_strerror(errno));
                break;
            }
        }
        ...
        if (!zmq_msg_more(&eventmsg)) {
            logit(
                NSLOG_RUNTIME_ERROR, TRUE, "Message should have been multipart, is only one part");
            break;
        }
        ...
        switch (event) {
            ...
            // case ZMQ_EVENT_CONNECT_DELAYED:
            // 	event_string = "NagMQ socket event on %.*s: synchronous connect failed, it's being
            // polled";
            // 	break;
            case ZMQ_EVENT_CONNECT_RETRIED:
                event_string =
                    "NagMQ socket event on %.*s: asynchronous connect / reconnection attempt (ivl: "
                    "%d)";
                break;
            case ZMQ_EVENT_LISTENING:
                event_string =
                    "NagMQ socket event on %.*s: socket bound to an address, ready to accept (fd: "
                    "%d)";
                break;
            case ZMQ_EVENT_BIND_FAILED:
                event_string =
                    "NagMQ socket event on %.*s: socket could not bind to an address (errno: %d)";
                break;
            case ZMQ_EVENT_ACCEPTED:
                event_string =
                    "NagMQ socket event on %.*s: connection accepted to bound interface (fd: %d)";
                break;
            case ZMQ_EVENT_ACCEPT_FAILED:
                event_string =
                    "NagMQ socket event on %.*s: could not accept client connection (errno: %d)";
                break;
            // This is super chatty. Commenting it out to reduce log chattyness
            // case ZMQ_EVENT_CLOSED:
            ...
            case ZMQ_EVENT_CLOSE_FAILED:
                event_string =
                    "NagMQ socket event on %.*s: connection couldn't be closed (errno: %d)";
                break;
        ...
        logit(NSLOG_INFO_MESSAGE,
              TRUE,
              event_string,
              zmq_msg_size(&addrmsg),
              (char*)zmq_msg_data(&addrmsg),
              value);
        zmq_msg_close(&addrmsg);
    }
    return 0;
...
void setup_sockmonitor(void* sock) {
    ...
    log_debug_info(
        DEBUGL_PROCESS, DEBUGV_BASIC, "Registered %s for socket monitoring on %d\n", channel, fd);
