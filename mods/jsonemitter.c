...
int payload_add_key(struct payload* po, char* key) {
    ...
    po->bufused += sprintf(po->json_buf + po->bufused, "\"%s\": ", key);
    return 1;
}
...
void payload_new_string(struct payload* po, char* key, char* val) {
    ...
    if (val == NULL) {
        ...
        po->bufused += sprintf(po->json_buf + po->bufused, "null, ");
        return;
    }
...
void payload_new_integer(struct payload* po, char* key, long long val) {
    ...
    po->bufused += sprintf(po->json_buf + po->bufused, "%lli, ", val);
}
...
void payload_new_boolean(struct payload* po, char* key, int val) {
    ...
    if (val > 0)
        po->bufused += sprintf(po->json_buf + po->bufused, "true, ");
    else
        po->bufused += sprintf(po->json_buf + po->bufused, "false, ");
}
...
void payload_new_double(struct payload* po, char* key, double val) {
    ...
    po->bufused += snprintf(po->json_buf + po->bufused, DBL_MAX_10_EXP, "%f", val) - 1;
    po->bufused += sprintf(po->json_buf + po->bufused, ", ");
}

void payload_new_timestamp(struct payload* po, char* key, struct timeval* tv) {
    if (!payload_add_key(po, key))
        return;
...
void payload_new_statestr(struct payload* ret, char* key, int state, int checked, int svc) {
