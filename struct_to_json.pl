...
while(<>) {
	...
	if(/^typedef struct (nebstruct_)?([^\s]+)_struct {/) {
		$curtype = $2;
		s/_//g;
		if(!/(?:servicestatus|hoststatus|servicecheck|hostcheck|programstatus|acknowledgement|statechange|host_check|service_check)/) {
			undef $curtype;
		 	next;
		}
#		print "Found struct $1\n";
		print "static json_t * parse_$curtype(nebstruct_${curtype}_data * obj) {\n";
